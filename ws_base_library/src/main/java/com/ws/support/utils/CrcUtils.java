...
public class CrcUtils {
     ...
     * XModem模式校验码
     */
    public static int crcXModem(byte[] bytes) {
        int crc = 0x00;
    ...
    //检验CRC
    public static boolean checkTemperateureCRC(String str, String prefix) {
        if (TextUtils.isEmpty(str) || str.length() < 12) {
            Logger.e("校验数据为空或者长度不对="+ str);
            return false;
        }
        ...
        //Logger.e("CRC = "+CRC + " 校验数据=" + check);
        String result = getCRC(toBytes(check));
        //Logger.e("校验结果="+result);
        return CRC.equalsIgnoreCase(result);
    }

    //检验CRC
    public static boolean checkCRC(String str, String CRC) {
        return CRC.equalsIgnoreCase(getCRC(str));
    }


    public static String getCRC(String str) {
        return getCRC(toBytes(str));
    }
