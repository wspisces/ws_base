...
public class MapCoordinateConvert {
	...
	public static double[] gcj02Towgs84Exact(double gcjLon, double gcjLat) {
		...
		while (true) {
			wgsLat = (mLat + pLat) / 2;
			wgsLon = (mLon + pLon) / 2;
			double[] tmp = MapCoordinateConvert.wgs84Togcj02(wgsLon, wgsLat);
			dLat = tmp[1] - gcjLat;
			dLon = tmp[0] - gcjLon;
	...
	public static double[] wgs84Tobd09(double wgLon, double wgLat) {
		double[] gcj = MapCoordinateConvert.wgs84Togcj02(wgLon, wgLat);
		double[] result = MapCoordinateConvert.gcj02Tobd09(gcj[0], gcj[1]);
		return result;
	}
	...
	public static double[] bd09Towgs84(double lon, double lat) {
		double[] gcj02 = MapCoordinateConvert.bd09Togcj02(lon, lat);
		double[] map84 = MapCoordinateConvert.gcj02Towgs84(gcj02[0], gcj02[1]);
		return map84;
	...
	public static double[] mapbarTowgs84(double lon, double lat) {
		...
		double x2 = - (((Math.cos(y1 / 100000)) * (x1 / 18000)) + ((Math.sin(x1 / 100000)) * (y1 / 9000))) + lon + ((lon > 0) ? 1 : -1);
		double y2 = - (((Math.sin(y1 / 100000)) * (x1 / 18000)) + ((Math.cos(x1 / 100000)) * (y1 / 9000))) + lat + ((lat > 0) ? 1 : -1);
	 ...
	 * @return
	 */
	public static double[] webMercatorToWgs84(double mercatorLon,
			double mercatorLat) {
		double[] result = new double[2];
